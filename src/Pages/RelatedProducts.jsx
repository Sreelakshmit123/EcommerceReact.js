...
import { RelatedProductsAPI } from '../Services/allAPIs';
...
function RelatedProducts() {
    ...
    const getRelatedProducts = async () => {
        try {
            const result = await RelatedProductsAPI();
                            ...
                            return (
                                <div key={index} className='cardslider'>
                                    <Card className='related-productcard mb-5'>
                                        <Card.Img className='relatedproduct-img' variant="top" src={item?.mainimage?.startsWith('http') ? item.mainimage : `${SERVER_URL}${item.mainimage}`} />
